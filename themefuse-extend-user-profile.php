   ...
   function ThemeFuse_Extend_User_Profile ( )
    {
        ...
        register_uninstall_hook(__FILE__,array($this, 'ThemeFuse_Extend_User_Profile_uninstall') );
    }
    ...
    private function ThemeFuse_Extend_User_Profile_get_Data ( $id )
    {
      ...
      $meta =  get_user_meta($id,'themefuse_extends_user_options',TRUE);
      return $meta;
    }
    ...
    public function ThemeFuse_Extend_User_Profile_save ()
        {
            ...
            update_user_meta( $userID, 'themefuse_extends_user_options', $inf);
        }
    ...
    {
        global $wpdb;
        $user_count = $wpdb->get_var("SELECT MAX(ID) FROM $wpdb->users");
        for ($i=1;i<=$user_count;$i++)
        {
            delete_user_meta($i, 'themefuse_extends_user_options');
        }
